...
namespace Gamekit2D
{
    ...
    public class PlayerCharacter : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            jump = FindObjectOfType<JumpButton>();
            _RangeButton = FindObjectOfType<RangeButton>();
        ...
        {
            bool faceLeft = PlayerInput.Instance.Horizontal.Value < 0f || _VariableJoystick.Horizontal < 0f;
            bool faceRight = PlayerInput.Instance.Horizontal.Value > 0f || _VariableJoystick.Horizontal > 0f;
        ...
        {
            float desiredSpeedPC = useInput ? PlayerInput.Instance.Horizontal.Value * maxSpeed * speedScale : 0f;
            float desiredSpeed = useInput ? _VariableJoystick.Horizontal * maxSpeed * speedScale : 0f;
        ...
        public void CheckForCrouching()
        {
            if (mobile)
            {
                m_Animator.SetBool(m_HashCrouchingPara, _VariableJoystick.Vertical < 0f); 
            }
            else
            {
        ...
        public void CheckForPushing()
        {
            ...
            {
                bool movingRight = PlayerInput.Instance.Horizontal.Value > float.Epsilon || _VariableJoystick.Horizontal > float.Epsilon;
                bool movingLeft = PlayerInput.Instance.Horizontal.Value < -float.Epsilon || _VariableJoystick.Horizontal < -float.Epsilon;
        ...
        {
            if (!PlayerInput.Instance.Jump.Held && m_MoveVector.y > 0.0f || jump.pressed && m_MoveVector.y > 0.0f)
            {
        ...
        {
            float desiredSpeedPC = PlayerInput.Instance.Horizontal.Value * maxSpeed;
            float desiredSpeed = _VariableJoystick.Horizontal * maxSpeed;
        ...
        public bool CheckForFallInput()
        {
            if (mobile)
            {
                return _VariableJoystick.Vertical < -float.Epsilon && jump.pressed;
            }
            else
            {
        ...
        public bool CheckForHoldingGun()
        {
            ...
            if (PlayerInput.Instance.RangedAttack.Held || _RangeButton.pressed)
            {
                holdingGun = true;
        ...
        public void CheckAndFireGun()
        {
            if (PlayerInput.Instance.RangedAttack.Held && m_Animator.GetBool(m_HashHoldingGunPara) ||
                _RangeButton.pressed && m_Animator.GetBool(m_HashHoldingGunPara))
            {
